...
Define centered_rank_expr function
...
labour_processed = (
    ...
    .with_columns(
        ...
        centered_rank=centered_rank_expr(
            pl.col('PDIFF')).over(['YEAR', 'MONTH'])
    )
)
labour_processed
```
...
(
    ...
    .select(['PDIFF', 'signed_rank', 'centered_rank'])
)
```

```{python}
(
    ggplot(labour_processed, aes(x="DATE_YMD", y="centered_rank", color="PDIFF")) +
    geom_point(shape='s')
)
```
